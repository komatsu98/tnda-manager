...
class ContractsImportBML implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            $status_code = $row[14];
            $product_code = $row[15];
            $submit_date = Util::parseDateExcel($row[20], 'm/d/Y', 'Y-m-d');
            $release_date = Util::parseDateExcel($row[21], 'm/d/Y', 'Y-m-d');
            $ack_date = Util::parseDateExcel($row[22], 'm/d/Y', 'Y-m-d');
            ...
            $contract_year = isset($row[19]) && $row[19] ? $row[19] : 0;
